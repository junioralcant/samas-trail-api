...
func (u *UserCreateController) Handle(ctx *gin.Context) {
	...
	createUser := models.NewUser(request.Name, request.Email, request.Phone, request.CPF, request.Team, request.City, request.Street, request.PostalCode, request.Paid, request.ShirtSize, request.Distance)
